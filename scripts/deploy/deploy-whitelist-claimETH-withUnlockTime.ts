...
const main = async () => {
  ...
  if (!address) {
    console.info(colorize(Color.blue, `Deploy WhitelistClaimETHWithUnlockTime`));
    ...
    console.info(`============================================================`);
    console.info(`================ Deploy WhitelistClaimETHWithUnlockTime ==================`);
    console.info(`============================================================`);
    const WhitelistClaimETHWithUnlockTime = await deployWhitelistClaimETHWithUnlockTime(
      claimMerkleRoot, claimStartTime, claimEndTime,
      getTxOverridesForNetwork(hre.network.name)
    );
    address = await WhitelistClaimETHWithUnlockTime.getAddress();
    console.info(`WhitelistClaimETHWithUnlockTime @ ${address}`);
  }
  ...
  if (!skipVerify) {
    try {
      await hre.run("verify:verify", {
        address: address,
        contract: "contracts/airdrop/WhitelistClaimETHWithUnlockTime.sol:WhitelistClaimETHWithUnlockTime",
